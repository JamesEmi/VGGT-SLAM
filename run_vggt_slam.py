...
parser.add_argument("--save_dir", type=str, default=".", help="Directory to save all output files (poses, point cloud, loop closures)")
...
def main():
    ...
    for image_name in tqdm(image_names):
        ...
        if len(image_names_subset) == args.submap_size + args.overlapping_window_size or image_name == image_names[-1]:
            ...
            if predictions.get("frames_lc_names"):
                for loop in predictions["detected_loops"]:
                    loop_closure_records.append({
                        "query_frame": predictions["frames_lc_names"][0],
                        "detected_frame": predictions["frames_lc_names"][1],
                        "similarity_score": loop.similarity_score,
                    })
            if args.vis_map:
    ...
    if args.log_results:
        ...
        solver.map.write_points_to_file(solver.graph, log_path.replace(".txt", "_points.pcd"))

        if not args.skip_dense_log:
            # Log the dense point cloud for each submap.
            solver.map.save_framewise_pointclouds(solver.graph, log_path.replace(".txt", "_logs"))

        if loop_closure_records:
            lc_path = log_path.replace(".txt", "_loop_closures.txt")
            with open(lc_path, "w") as f:
                for rec in loop_closure_records:
                    f.write(f"{rec['query_frame']} {rec['detected_frame']} {rec['similarity_score']:.6f}\n")
            print(f"Saved {len(loop_closure_records)} loop closure pairs to {lc_path}")
